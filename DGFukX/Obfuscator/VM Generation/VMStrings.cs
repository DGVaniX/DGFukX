...
namespace DGFukX.Obfuscator.VM_Generation
{
	public static class VMStrings
	{
		...
		public static List<string> VM_MemeStrings = new List<string>
		{
			"#Valoare",
			"https://www.youtube.com/watch?v=ixk4Gv5aiK4",
			"Daca vezi asta, SA ITI IAU FAMILIA IN PULA",
			"Recunoaste, sunteti fanii mei, mi-ai tine coaiele in dinti ca niste caini dresati",
			"Vreti sa fiti ca mine cand o sa cresteti, nu-i asa vladutzu mami? ;)",
			"Macar daca tot furati de la mine ca pe voi nu va duce capul, aratati putin respect si nu mai vorbiti urat"
		};
...
local function Wrap(Chunk, Upvalues, Env)
	...
	return function(...)
		...
		if not A[1] then
			...
			error('ERROR IN DGFUKX SCRIPT [LINE ' .. line .. ']:' .. A[2])
		else
			return Unpack(A, 2, B)
		end;
	end;
